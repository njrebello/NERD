...
public:
	...
	void initialize()
	{
		// initialize all DAA (distance between A groups) to MAX possible distance
		...
		// initialize all DBB (distance between B groups) to MAX possible distance
		...
		// initialize all DAB (distance between A and B groups) to MAX possible distance
		...
		// Use the Kuhn monomer template initialize distances between functional groups on the same molecule
	...
	vector <int> choose()
	{
		// choose a functional group to react based on random number generator
	...
	{
		int num_steps;
		// conversion is based on smaller number of functional groups
		if (total_A < total_B)
		{
		...
		for (int i = 0; i < num_steps; i++)
		{
			...
			sum_A[chosen_A] = 0.0;
			// set reacted indices appropriately
...
int main(int argc, char* argv[])
{
	...
	ofstream data_file("Data-File-Conc-Mol1-" + std::to_string(int(conc[0])) + "mM.txt");
