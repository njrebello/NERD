...
from polymersearch.search_tools import graph_traversal_match, generate_NetworkX_graphs, identify_cycles

def M23_validation():
    data = pd.read_excel("NERD-Database.xlsx", sheet_name = "BigSMILES_Eq")

    cases = dict()   
    for index, row in data.iterrows():
        case = row["Case"]
        bigsmiles = row["BigSMILES"]
        # populate dictionary of cases mapping number to list of BigSMILES
        if case not in cases:
            cases[case] = []
        cases[case].append(bigsmiles)
    
    for case in cases:
        for bigsmiles in cases[case]:
            information = {bigsmiles: 
            {0: {'object': bigsmiles, 'Da': 0, 'kDa': 10, 'DP': 0, 'Mo': 0, 'b': 0}, 
            "concentration": 20, "number": 20000}
            }

            print(bigsmiles)
            N, b = coarse_grain_arm(bigsmiles, bigsmiles, 298, information[bigsmiles][0], True)
            print(b)
...
def substructure_search_rxns(smiles):
    ...
    for d in databases:
        df = pd.read_excel('M23/NERD-Database.xlsx', sheet_name = d) 
        SMARTS = df['SMARTS'] 
        for rxn in SMARTS:
            s = rxn.split(">")
            reactants = s[0].split(".")
...
def coarse_grain_arm(smiles, object, input_T, information, sub):
    ...
    def get_Mo_b(): 
        data = pd.read_excel("M23/NERD-Database.xlsx", sheet_name = "Kuhn")
        ...
        def sim():
            for i in range(len(BigSMILES)):
                ...
                tanimoto = DataStructs.TanimotoSimilarity(a, b)

                # graphs = generate_NetworkX_graphs(stochastic_object, is_bigsmarts = False) 
                # group_cycles, individual_clusters, treat_as_cycle = identify_cycles(graphs)
                # v1 = ECFP(graphs = graphs, individual_clusters = individual_clusters)

                # graphs = generate_NetworkX_graphs(BigSMILES[i], is_bigsmarts = False) 
                # group_cycles, individual_clusters, treat_as_cycle = identify_cycles(graphs)
                # v2 = ECFP(graphs = graphs, individual_clusters = individual_clusters)
                # cosine, dice, manhattan, tanimoto = pairwise_similarity(v1, v2) 

                found.append((BigSMILES[i], (1 - tanimoto), abs(T[i] - input_T), Mk[i], k[i]))
        if sub:
            for i in range(len(BigSMILES)):
